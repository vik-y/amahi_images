amahi-download osticket-1.10.sql.gz 901e745a307093d2a25a1845aea59692f87c55bd;

# Check for unpack folder
if [ -d unpack ]; then
  rm -rf html;
  mv unpack/upload html;
  mv unpack/scripts html/;
  rm -rf unpack;
fi

cat > html/include/ost-config.php << 'EOF'
<?php
/*********************************************************************
    ost-config.php
...
define('SECRET_SALT','2FH93q0iU1CGaNGw0lyNw8cBdqmyYG9z');

#Default admin email. Used only on db connection issues and related alerts.
define('ADMIN_EMAIL','admin@yourisp.com');
...
chmod 444 html/include/ost-config.php;

zcat osticket-1.10.sql.gz |mysql -u$HDA_DB_USERNAME -p$HDA_DB_PASSWORD $HDA_DB_DBNAME;

cat > Dockerfile << 'EOF'
FROM vikasy/slim_php5

RUN apk update && \
    apk add --no-cache php5-mysqli php5-xml php5-json php5-phar php5-intl
EOF
